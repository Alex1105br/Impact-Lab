...
int main() {
    ...
    unsigned char* img = stbi_load("images/dead_cells.jpeg", &width, &height, &channels, CHANNEL_NUM);
    if (img == NULL) {
    ...
    auto duration_multi_thread = std::chrono::duration_cast<std::chrono::nanoseconds>(stop_multi_thread - start_multi_thread);
    ...
    cout << "Single-threaded image processing time: " << duration_single_thread.count() << " nanoseconds" << endl;
    cout << "Multithreaded image processing time: " << duration_multi_thread.count() << " nanoseconds" << endl;
